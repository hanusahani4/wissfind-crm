...
export class ForgotPasswordComponent {
  ...
  async sendOtp(){
    ...
    this.loading=true;const result=await this.auth.sendPasswordResetOtp(this.phone);this.loading=false;
  ...
  async verifyOtp(){
    ...
    this.loading=true;const {error}=await this.auth.verifyPasswordResetOtp(this.phone,this.otp.trim());this.loading=false;
  ...
  async resendOtp(){
    ...
    const result=await this.auth.sendPasswordResetOtp(this.phone);this.loading=false;
    if(result.error){this.error=result.error.message;return;}
  ...
  async changePassword(){
    ...
    this.loading=true;const {error}=await this.auth.updatePassword(this.password);this.loading=false;
    if(error){this.error=error.message;return;}
