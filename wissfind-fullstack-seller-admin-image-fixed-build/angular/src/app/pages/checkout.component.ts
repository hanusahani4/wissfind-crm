  ...
  <main class="page"><div class="container">
    ...
    <div *ngIf="cart.cart().length && !placed; else state" class="checkout-grid">
      <section class="card form">
        ...
        <p class="cod-limit-note" *ngIf="!cart.codAllowed()">{{cart.codUnavailableReason()}}</p>
        <div class="payment">
          <label [class.active-payment]="cart.paymentMethod() === 'COD'" [class.disabled-payment]="!cart.codAllowed()">
            <input type="radio" name="payment" value="COD" [checked]="cart.paymentMethod() === 'COD'" [disabled]="!cart.codAllowed()" (change)="selectPayment('COD')">
            <span><b>Cash on delivery</b><small *ngIf="cart.codAllowed()">Pay ₹{{cart.codTotal() | number}} on delivery · Delivery ₹{{cart.shippingFor('COD') | number}}</small><small class="cod-unavailable" *ngIf="!cart.codAllowed()">Unavailable for orders above the maximum COD value.</small></span>
          </label>
