  ...
  template: `
    <section class="homepage-sections" *ngIf="sections.length">
      <div class="section" *ngFor="let section of sections">
        <div class="section-head"><div><div class="eyebrow">CURATED FOR YOU</div><h2>{{ section.title }}</h2></div><a *ngIf="section.showViewAll" routerLink="/" class="view-all">View all →</a></div>
        <div class="product-row">
          <article class="card" *ngFor="let p of section.products">
            <a [routerLink]="['/product', p.id]" class="image"><img [src]="imageUrl(p.image)" [alt]="p.name" loading="lazy"><span class="badge" *ngIf="p.salePrice || p.oldPrice">SALE</span></a>
            <div class="info"><div class="meta"><span>{{ p.category }}</span><span>★ {{ p.rating || 0 }}</span></div><a [routerLink]="['/product', p.id]"><h3>{{ p.name }}</h3></a>
              <div class="social"><button (click)="like($event,p.id)" [class.liked]="reviews.isProductLiked(p.id)">{{ reviews.isProductLiked(p.id) ? '♥' : '♡' }} {{ reviews.getProductLikeCount(p.id) }}</button><a [routerLink]="['/product',p.id]">Reviews →</a></div>
              <div class="bottom"><div><b>₹{{ (p.salePrice || p.price) | number }}</b><del *ngIf="p.salePrice || p.oldPrice">₹{{ p.oldPrice || p.price | number }}</del></div><button class="cart" (click)="add($event,p)" [disabled]="!p.stock">Add to cart</button></div>
            </div>
          </article>
        </div>
      </div>
...
export class HomepageSectionsComponent {
  ...
  async load(){try{const data:any=await this.api.get('/homepage');this.sections=Array.isArray(data?.sections)?data.sections:[];}catch{this.sections=[];}}
