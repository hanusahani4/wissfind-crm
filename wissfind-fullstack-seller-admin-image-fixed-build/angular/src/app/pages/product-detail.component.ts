...
@Component({
  ...
  <main class="page" *ngIf="product">
    <div class="container">
      ...
      <section class="detail">
        <div class="gallery" [class.single-image]="product.images?.length <= 1">
          ...
          <div class="main-image" (mouseenter)="pauseAutoSlide()" (mouseleave)="resumeAutoSlide()"><button class="slide-arrow prev" type="button" (click)="previousImage()" aria-label="Previous image">‹</button><img [src]="selectedImage" [alt]="product.name" (load)="fitMainImage($event)"><button class="slide-arrow next" type="button" (click)="nextImage()" aria-label="Next image">›</button><div class="slide-progress"><span *ngFor="let image of product.images; let i = index" [class.active]="i === selectedIndex"></span></div><span class="image-count">{{ selectedIndex + 1 }} / {{ product.images.length }}</span></div>
        </div>
        <div class="copy">
    ...
    <section class="related-products-section" *ngIf="relatedProducts.length"><div class="related-heading"><div><div class="eyebrow">You may also like</div><h2>Related products</h2></div><button type="button" class="view-all-category" (click)="openAllCategoryProducts()">View all →</button></div><div class="related-grid"><a class="related-card" *ngFor="let related of relatedProducts" [routerLink]="['/product', related.id]" [state]="{product:related}"><div class="related-image"><span class="sale-badge" *ngIf="related.oldPrice">SALE</span><img [src]="related.image" [alt]="related.name"><button type="button" class="related-like" (click)="$event.preventDefault(); $event.stopPropagation(); toggleRelatedLike(related.id)">{{ isRelatedLiked(related.id) ? '♥' : '♡' }}</button></div><div class="related-meta"><span class="related-category">{{ related.category }}</span><span class="related-rating">★ {{ related.rating }}</span></div><h3>{{ related.name }}</h3><div class="related-price"><strong>₹{{ related.price | number }}</strong><del *ngIf="related.oldPrice">₹{{ related.oldPrice | number }}</del></div></a></div></section>
    <section id="category-products" class="category-products-section" *ngIf="showAllCategoryProducts"><div class="category-products-heading"><div><div class="eyebrow">Shop this category</div><h2>More {{ product.category }} products</h2><p>Showing products from the same category as {{ product.name }}.</p></div><span class="category-count">{{ categoryProducts.length }} products</span></div><div class="category-products-grid"><a class="category-product-card" *ngFor="let item of pagedCategoryProducts" [routerLink]="['/product', item.id]" [state]="{product:item}"><div class="category-product-image"><span class="sale-badge" *ngIf="item.oldPrice">SALE</span><img [src]="item.image" [alt]="item.name"></div><div class="related-meta"><span class="related-category">{{ item.category }}</span><span class="related-rating">★ {{ item.rating }}</span></div><h3>{{ item.name }}</h3><div class="related-price"><strong>₹{{ item.price | number }}</strong><del *ngIf="item.oldPrice">₹{{ item.oldPrice | number }}</del></div></a></div><nav class="category-pagination" *ngIf="categoryPageCount > 1" aria-label="Category product pages"><button type="button" class="pagination-button" [disabled]="categoryPage === 1" (click)="setCategoryPage(categoryPage - 1)">←</button><button type="button" class="pagination-button" *ngFor="let page of categoryPageNumbers" [class.active]="categoryPage === page" (click)="setCategoryPage(page)">{{ page }}</button><button type="button" class="pagination-button" [disabled]="categoryPage === categoryPageCount" (click)="setCategoryPage(categoryPage + 1)">→</button></nav></section>
  </main>
  `,
  styles: [`
...
export class ProductDetailComponent implements OnInit, OnDestroy {
  ...
  fitMainImage(event: Event) {
    const image = event.target as HTMLImageElement | null;
    const container = image?.parentElement as HTMLElement | null;
    if (!image || !container || !image.naturalWidth || !image.naturalHeight) return;
    const ratio = image.naturalWidth / image.naturalHeight;
    container.style.aspectRatio = ratio >= 1.05 ? String(Math.min(1.7, Math.max(0.85, ratio))) : String(Math.min(0.95, Math.max(0.68, ratio)));
  }
