...
public class ReviewController {
 final ReviewRepository reviews; final ProductRepository products; final UserRepository users; final OrderRepository orders;
 public ReviewController(ReviewRepository r,ProductRepository p,UserRepository u,OrderRepository o){reviews=r;products=p;users=u;orders=o;}
 ...
 public Map<String,Object> eligibility(@PathVariable Long productId){
  ...
  boolean alreadyReviewed=reviews.existsByProductIdAndCustomerId(productId,customerId);
  return Map.of("canReview",purchased&&!alreadyReviewed,"purchased",purchased,"alreadyReviewed",alreadyReviewed);
 }
 ...
 public Map<String,Object> add(@PathVariable Long productId,@RequestBody ReviewRequest req){
  ...
  if(orders.countPurchasedProduct(u.id,productId)==0) throw new IllegalArgumentException("You can review only products you have ordered");
 ...
 private void recalculate(Product p) {
  ...
  p.reviews=all.size();
  if(all.isEmpty()){p.rating=0.0;return;}
  double average=all.stream().mapToInt(x->x.rating).average().orElse(0.0);
