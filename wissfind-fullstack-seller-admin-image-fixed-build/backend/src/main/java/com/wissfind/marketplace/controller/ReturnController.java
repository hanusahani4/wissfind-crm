...
@RestController @RequestMapping("/api/returns") public class ReturnController { final ReturnRequestRepository repo; final OrderRepository orders; final UserRepository users; public ReturnController(ReturnRequestRepository r,OrderRepository o,UserRepository u){repo=r;orders=o;users=u;}
 ...
 @GetMapping("/mine/paged") @PreAuthorize("hasAnyRole('CUSTOMER','SELLER')") @org.springframework.transaction.annotation.Transactional(readOnly=true)
 ...
 @PostMapping @PreAuthorize("hasAnyRole('CUSTOMER','SELLER')") public ReturnRequest create(@RequestBody ReturnRequest r){r.id=null;r.order=orders.findById(r.order.id).orElseThrow();r.customer=users.findById(CurrentUser.id()).orElseThrow();if(repo.existsByOrderId(r.order.id))throw new IllegalArgumentException("Return request already exists for this order");r.status=ReturnRequest.Status.REQUESTED;r.requestType=ReturnRequest.RequestType.RETURN;return repo.save(r);}
