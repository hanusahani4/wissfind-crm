...
public class OrderController {
    ...
    public org.springframework.data.domain.Page<Order> minePaged(
        ...
        if (status == null || status.isBlank() || "All".equalsIgnoreCase(status)) return repo.findByCustomerId(CurrentUser.id(), pageable);
    ...
    @Transactional(readOnly = true)
    public List<Order> seller() { return repo.findBySellerIdOrderByCreatedAtDesc(CurrentUser.id()); }
    ...
    public Order create(@RequestBody Map<String, Object> body) {
        ...
        if (requestedItems.isEmpty() && !productIds.isEmpty()) {
            for (Long productId : productIds) requestedItems.add(new LinkedHashMap<>(Map.of("productId", productId, "quantity", 1)));
        }
        if (requestedItems.isEmpty()) throw new IllegalArgumentException("At least one product is required to create an order");
        productIds = requestedItems.stream().map(item -> longValue(item.get("productId"))).filter(Objects::nonNull).toList();

        User seller = null;
        if (requestedSellerId != null) {
            seller = users.findById(requestedSellerId).orElseThrow(() -> new IllegalArgumentException("Seller not found"));
            if (seller.role != User.Role.SELLER) throw new IllegalArgumentException("Selected user is not a seller");
        }
        if (!productIds.isEmpty()) {
            Set<Long> sellerIds = new LinkedHashSet<>();
            for (Long productId : productIds) {
                Product product = products.findById(productId).orElseThrow(() -> new IllegalArgumentException("Product not found: " + productId));
                if (product.seller == null) throw new IllegalArgumentException("Product has no seller: " + productId);
                sellerIds.add(product.seller.id);
            }
            if (sellerIds.size() > 1) throw new IllegalArgumentException("Cart contains products from multiple sellers. Please checkout one seller at a time.");
            Long actualSellerId = sellerIds.iterator().next();
            if (seller != null && !Objects.equals(seller.id, actualSellerId)) throw new IllegalArgumentException("Selected seller does not own the cart products");
            seller = users.findById(actualSellerId).orElseThrow(() -> new IllegalArgumentException("Seller account not found"));
        }
        if (seller == null) throw new IllegalArgumentException("Unable to determine seller for this order. Refresh products and try again.");

        Order order = new Order();
        order.customer = customer;
        order.seller = seller;
        order.orderNumber = stringValue(body.get("orderNumber"));
        if (order.orderNumber == null || order.orderNumber.isBlank()) order.orderNumber = "WF-" + System.currentTimeMillis();
        if (repo.existsByOrderNumber(order.orderNumber)) throw new IllegalArgumentException("Order number already exists");
        String paymentMethod = stringValue(body.get("paymentMethod"));
        if (!"RAZORPAY".equalsIgnoreCase(paymentMethod) && !"COD".equalsIgnoreCase(paymentMethod)) throw new IllegalArgumentException("Payment method must be COD or RAZORPAY");
        order.paymentMethod = paymentMethod.toUpperCase(Locale.ROOT);

        Long shippingAddressId = longValue(body.get("shippingAddressId"));
        if (shippingAddressId == null) throw new IllegalArgumentException("Please select a shipping address");
        CustomerAddress shippingAddress = addresses.findByIdAndCustomerId(shippingAddressId, customer.id).orElseThrow(() -> new IllegalArgumentException("Selected shipping address is invalid"));
        ...
        for (Map<String, Object> requestedItem : requestedItems) {
            ...
            if (quantity < 1) throw new IllegalArgumentException("Quantity must be at least 1 for product: " + productId);
            Object requestedVariant = requestedItem.get("variant");
            ProductSizeVariant variant = findVariant(product.id, requestedVariant);
            int availableStock = variant != null ? Math.max(0, variant.stock) : Math.max(0, product.stock);
            if (availableStock < quantity) throw new IllegalArgumentException("Insufficient stock for product: " + product.name);
            ...
            if (variant != null) {
                ...
                if (variant.colorVariant != null && variant.colorVariant.images != null && !variant.colorVariant.images.isEmpty()) item.image = variant.colorVariant.images.get(0);
        ...
        BigDecimal serverSubtotal = order.items.stream().map(i -> i.price.multiply(BigDecimal.valueOf(i.quantity))).reduce(BigDecimal.ZERO, BigDecimal::add);
        String couponCode = stringValue(body.get("couponCode"));
        BigDecimal couponDiscount = "WISS10".equalsIgnoreCase(couponCode) ? serverSubtotal.multiply(new BigDecimal("0.10")).setScale(0, java.math.RoundingMode.HALF_UP) : BigDecimal.ZERO;
        BigDecimal payableProducts = serverSubtotal.subtract(couponDiscount).max(BigDecimal.ZERO);
        ShippingSettings shippingSettings = shippingSettingsRepo.findAll().stream().findFirst().orElseGet(() -> shippingSettingsRepo.save(new ShippingSettings()));
        if ("COD".equals(order.paymentMethod)) {
            if (!shippingSettings.codEnabled) throw new IllegalArgumentException("Cash on delivery is currently unavailable. Please choose prepaid payment.");
            if (payableProducts.compareTo(shippingSettings.codMaxOrderAmount) > 0) throw new IllegalArgumentException("COD is available only up to ₹" + shippingSettings.codMaxOrderAmount.stripTrailingZeros().toPlainString() + ". Please choose prepaid payment for this order.");
        }
        BigDecimal serverShipping = payableProducts.compareTo(shippingSettings.freeShippingThreshold) >= 0 ? BigDecimal.ZERO : ("COD".equals(order.paymentMethod) ? shippingSettings.codShippingCharge : shippingSettings.prepaidShippingCharge);
    ...
    public Order cancel(@PathVariable Long id, @RequestBody(required = false) Map<String, String> body) {
        ...
        if (order.customer == null || !order.customer.id.equals(CurrentUser.id())) throw new IllegalArgumentException("You can cancel only your own order");
        if ("Cancelled".equalsIgnoreCase(order.deliveryStatus)) throw new IllegalArgumentException("Order is already cancelled");
        if (!"Processing".equalsIgnoreCase(order.deliveryStatus)) throw new IllegalArgumentException("This order can no longer be cancelled. Please use the return option after delivery.");
        ...
        order.paymentStatus = "PAID".equalsIgnoreCase(order.paymentStatus) ? "REFUND_PENDING" : "CANCELLED";
        restoreStock(order);
        Order saved = repo.save(order);
        if (!returnRequests.existsByOrderId(order.id)) {
            var rr = new com.wissfind.marketplace.entity.ReturnRequest();
            rr.order = saved; rr.customer = customerForCurrentUser(); rr.reason = reason.trim(); rr.requestType = com.wissfind.marketplace.entity.ReturnRequest.RequestType.CANCELLATION;
            rr.refundAmount = "REFUND_PENDING".equalsIgnoreCase(saved.paymentStatus) ? saved.total : java.math.BigDecimal.ZERO;
            rr.status = com.wissfind.marketplace.entity.ReturnRequest.Status.REQUESTED; returnRequests.save(rr);
        }
        return saved;
    }
    ...
    @PreAuthorize("hasRole('SELLER')")
    @Transactional
    public Order rejectBySeller(@PathVariable Long id, @RequestBody(required = false) Map<String, String> body) {
        Order order = repo.findById(id).orElseThrow(() -> new IllegalArgumentException("Order not found"));
        if (order.seller == null || !Objects.equals(order.seller.id, CurrentUser.id())) throw new IllegalArgumentException("You can reject only your own orders");
        if (!"Processing".equalsIgnoreCase(order.deliveryStatus)) throw new IllegalArgumentException("Only Processing orders can be rejected");
        String reason = body == null ? null : body.get("reason");
        ...
        if (!returnRequests.existsByOrderId(saved.id)) {
            var rr = new com.wissfind.marketplace.entity.ReturnRequest();
            rr.order = saved; rr.customer = saved.customer; rr.reason = "Seller rejected: " + reason.trim(); rr.requestType = com.wissfind.marketplace.entity.ReturnRequest.RequestType.CANCELLATION;
            rr.refundAmount = "REFUND_PENDING".equalsIgnoreCase(saved.paymentStatus) ? saved.total : java.math.BigDecimal.ZERO;
            rr.status = com.wissfind.marketplace.entity.ReturnRequest.Status.REQUESTED; returnRequests.save(rr);
        }
        return saved;
    ...
    public Order status(@PathVariable Long id, @RequestParam String value) {
        ...
        if (CurrentUser.role().equals("SELLER") && (order.seller == null || !order.seller.id.equals(CurrentUser.id()))) throw new IllegalArgumentException("Not your order");
        if ("Cancelled".equalsIgnoreCase(order.deliveryStatus)) throw new IllegalArgumentException("Cancelled orders cannot be moved to another delivery status");
        if (value == null || value.isBlank()) throw new IllegalArgumentException("Order status is required");
        order.deliveryStatus = value.trim(); return repo.save(order);
    }
    ...
    private void restoreStock(Order order) {
        ...
        for (OrderItem item : order.items) {
            ...
            products.findById(item.productId).ifPresent(product -> {
                ...
                if (variant != null) { variant.stock += quantity; product.stock = totalVariantStock(product.id); }
    ...
    private ProductSizeVariant findVariant(Long productId, Object rawVariant) {
        ...
        List<ProductSizeVariant> sizes = productVariants.stream()
                ...
                .flatMap(v -> v.sizes == null ? java.util.stream.Stream.empty() : v.sizes.stream()).toList();
        if (!sku.isBlank()) { Optional<ProductSizeVariant> bySku = sizes.stream().filter(v -> equalsIgnoreCase(v.sku, sku)).findFirst(); if (bySku.isPresent()) return bySku.get(); }
        if (!size.isBlank()) { Optional<ProductSizeVariant> bySize = sizes.stream().filter(v -> equalsIgnoreCase(v.size, size)).findFirst(); if (bySize.isPresent()) return bySize.get(); }
    ...
    private int totalVariantStock(Long productId) {
        return colorVariants.findByProductIdOrderByIdAsc(productId).stream().filter(Objects::nonNull).flatMap(v -> v.sizes == null ? java.util.stream.Stream.empty() : v.sizes.stream()).mapToInt(v -> Math.max(0, v.stock)).sum();
    }
    ...
    private String variantValue(Object rawVariant, String key) {
        ...
        var quoted = java.util.regex.Pattern
    ...
    private int intValue(Object value) { if (value == null) return 0; if (value instanceof Number n) return n.intValue(); try { return Integer.parseInt(String.valueOf(value)); } catch (NumberFormatException ex) { return 0; } }
    private List<Map<String, Object>> mapList(Object value) { if (!(value instanceof Collection<?> collection)) return new ArrayList<>(); List<Map<String, Object>> result = new ArrayList<>(); for (Object item : collection) if (item instanceof Map<?, ?> map) { Map<String, Object> copy = new LinkedHashMap<>(); map.forEach((key, val) -> copy.put(String.valueOf(key), val)); result.add(copy); } return result; }
    private Long longValue(Object value) { if (value == null) return null; if (value instanceof Number n) return n.longValue(); try { return Long.parseLong(String.valueOf(value)); } catch (NumberFormatException ex) { return null; } }
    private List<Long> longList(Object value) { if (!(value instanceof Collection<?> collection)) return new ArrayList<>(); List<Long> result = new ArrayList<>(); for (Object item : collection) { Long id = longValue(item); if (id != null) result.add(id); } return result; }
    private BigDecimal decimal(Object value) { if (value == null) return BigDecimal.ZERO; try { return new BigDecimal(String.valueOf(value)); } catch (NumberFormatException ex) { return BigDecimal.ZERO; } }
